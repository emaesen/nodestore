...
var db;
//var connectUri = "mongodb://localhost:27017/winedb";
var connectUri = "mongodb://heroku_app16879190:50oslnevuuetu5a0odjok76lau@ds035448.mongolab.com:35448/heroku_app16879190";

//'mongodb://heroku_app16879190:50oslnevuuetu5a0odjok76lau@ds035448.mongolab.com:35448/heroku_app16879190'
// var server = new Server('localhost', 27017, {auto_reconnect: true});
// db = new Db('winedb', server, {safe: true});

MongoClient.connect(connectUri, function(err, dbinst) {
	db = dbinst;
	if(!err) {
		console.log("Connected to database " + connectUri);
		db.collection('wines', function(err, collection) {
			if (err) {
				console.log("The 'wines' collection doesn't exist. Creating it with sample data...");
				populateDB();
			} else {
				console.log("Got access to 'wines' collection");
				collection.count(function(err,cnt){
					console.log("Collection contains " + cnt + " items");
				});
			}
		});
	} else {
...
exports.findAll = function(req, res) {
	console.log('Retrieving wine collection');
	db.collection('wines', function(err, collection) {
...
exports.updateWine = function(req, res) {
	...
	db.collection('wines', function(err, collection) {
		collection.update({'_id':new BSON.ObjectID(id)}, wine, function(err, result) {
			if (err) {
...
var populateDB = function() {
	...
	db.collection('wines', function(err, collection) {
		collection.insert(wines, function(err, result) {});
	});
